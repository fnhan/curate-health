...
// Query: {  "page": *[_type == "ourTeam" && pageActive == true][0]{    heroSection{      heroTitle,      heroParagraph    },      seo{    pageTitle,    pageDescription,    socialMeta{      title,      description,      ogImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      },      twitterImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      }    }  }  },  "practitioners": *[_type == "practitioner" && isActive == true] | order(name asc){    name,    "slug": slug.current,    credentials,    // The whole image object, not just a URL, so urlForPractitionerPhoto can    // read the hotspot and crop an editor set in the Studio.    photo{      ...,      "url": asset->url,      alt    }  }}
