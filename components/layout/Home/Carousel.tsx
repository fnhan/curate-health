...
export const SwipeCarousel = () => {
  ...
  useEffect(() => {
    const intervalRef = setInterval(() => {
      const x = dragX.get();
      if (x === 0) {
        setImgIndex((prevIndex) => (prevIndex === products.length - 1 ? 0 : prevIndex + 1));
      }
    }, AUTO_DELAY);
...
const Dots = ({ imgIndex, setImgIndex }) => {
  ...
  const numImagesLargeScreen = 3; // Number of images for the largest screen
  const numImagesMediumScreen = 2; // Number of images for the second largest screen
  ...
  const calculateNumDots = () => {
    if (typeof window !== 'undefined') {
      ...
      if (screenWidth >= 1200) {
        return Math.ceil(totalImages / numImagesLargeScreen);
      } else if (screenWidth >= 768) {
        return Math.ceil(totalImages / numImagesMediumScreen);
      } else {
        return totalImages;
      }
    }
        ...
        return (
          <button
            ...
            className={`h-3 w-3 rounded-full transition-colors ${
              actualIdx === imgIndex ? 'bg-primary' : 'bg-secondary'
            }`}
          />
        );
      })}
