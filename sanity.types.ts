...
// Query: {  "page": *[_type == "ourTeam" && pageActive == true][0]{    heroSection{      heroTitle,      heroParagraph    },      seo{    pageTitle,    pageDescription,    socialMeta{      title,      description,      ogImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      },      twitterImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      }    }  }  },  // In the order set on the team page document, the drag-to-reorder list  // from #205. #238 sorted by name instead, which moved Safa ahead of  // Dr. Gabriele and Andrew ahead of Ariel on a page whose order was chosen.  // isActive is selected rather than filtered here, and the component drops  // inactive and dangling references.  "practitioners": *[_type == "ourTeam" && pageActive == true][0].practitioners[]->{    name,    isActive,    "slug": slug.current,    credentials,    // The whole image object, not just a URL, so urlForPractitionerPhoto can    // read the hotspot and crop an editor set in the Studio.    photo{      ...,      "url": asset->url,      alt    }  }}
