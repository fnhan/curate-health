...
export type OurProgramsSection = {
  _id: string;
  _type: "ourProgramsSection";
  _createdAt: string;
  _updatedAt: string;
  _rev: string;
  sectionTitle?: string;
...
export type ServiceLifestyle = {
  ...
  benefits?: Array<{
    ...
    tint_percentage?: string;
    tint_percentage_hover?: string;
    _type: "benefit";
    _key: string;
  }>;
  block_7_image?: {
...
export type Practitioner = {
  ...
  commonlyTreatsLabel?: string;
  commonlyTreats?: Array<string>;
  janeBookingUrl?: string;
  bookingNote?: string;
  bookingCtaLabel?: string;
  bookingCtaTarget?: "curateLifestyleReferralForm" | "curateLifestyleProgram";
  seo?: Seo;
};
...
// Query: *[_type == "treatments" && isActive == true && treatmentSlug.current == $slug][0] {  _updatedAt,  title,  treatmentSlug,  "serviceName": service->title,  "serviceSlug": service->slug.current,  heroImage {    asset->{      url,          },    heroAlt  },  intro {    subtitle,    introParagraph  },  quoteContent,  janeBookingUrl,  // Who provides this, for the block at the foot of the page. Dereferenced  // rather than stored on the page, so a practitioner switched off in the  // Studio drops out of every service that lists them.  practitioners[]->{    name,    "slug": slug.current,    credentials,    // The whole image object, not just a URL, so urlForPractitionerPhoto can    // read the hotspot and crop an editor set in the Studio.    photo{      ...,      "url": asset->url,      alt    },    isActive  },  additionalSections[] {    sectionTitle,    sectionParagraph,    sectionImage {      "image": image.asset->url,      alt    }  },    benefits {    title,    benefitsList[] {      title,      subtitle    }  },  cta {    ctaBg {      asset->{        url,        metadata {          dimensions        }      }    },    ctaBgAlt,    ctaTitle,    ctaText,    ctaButtonText  },    seo{    pageTitle,    pageDescription,    socialMeta{      title,      description,      ogImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      },      twitterImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      }    }  }}
...
// Variable: PRACTITIONER_BY_SLUG_QUERY
// Query: *[_type == "practitioner" && isActive == true && slug.current == $slug][0]{  name,  "slug": slug.current,  credentials,  languages,  commonlyTreats,  commonlyTreatsLabel,  fullBio,  // The whole image object, not just a URL, so urlForPractitionerPhoto can  // read the hotspot and crop an editor set in the Studio.  photo{    ...,    "url": asset->url,    alt  },  janeBookingUrl,  bookingNote,  bookingCtaLabel,  bookingCtaTarget,  "provides": *[_type == "treatments" && isActive == true && references(^._id)]    | order(title asc){      title,      "slug": treatmentSlug.current,      "serviceSlug": service->slug.current,      "serviceName": service->title    },    seo{    pageTitle,    pageDescription,    socialMeta{      title,      description,      ogImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      },      twitterImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      }    }  }}
export type PRACTITIONER_BY_SLUG_QUERYResult = {
  name: string | null;
  slug: string | null;
  credentials: Array<string> | null;
  languages: Array<string> | null;
  commonlyTreats: Array<string> | null;
  commonlyTreatsLabel: string | null;
  fullBio: Array<
    | {
        children?: Array<{
          marks?: Array<string>;
  ...
  bookingCtaTarget:
    | "curateLifestyleProgram"
    | "curateLifestyleReferralForm"
    | null;
  provides: Array<{
    title: string | null;
    slug: string | null;
...
// Variable: PRACTITIONER_SLUGS_QUERY
// Query: *[_type == "practitioner" && isActive == true && defined(slug.current)].slug.current
export type PRACTITIONER_SLUGS_QUERYResult = Array<string | null>;
// Variable: OUR_TEAM_PAGE_QUERY
// Query: *[_type == "ourTeam" && pageActive == true][0]{  heroSection{    heroTitle,    heroParagraph  },  teamMembers[!(name in *[_type == "practitioner" && isActive == false].name)] {    name,    role,    bio,    image {      asset-> {        url      }    }  },    seo{    pageTitle,    pageDescription,    socialMeta{      title,      description,      ogImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      },      twitterImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      }    }  }}
...
export type SITEMAP_QUERYResult = {
  ...
  practitioners: Array<string | null>;
  story: Array<{
    _id: string;
  }>;
  missionValues: Array<{
...
// Query: *[_type == "serviceLifestyle" && slug.current == $slug][0]{    title,    "slug": slug.current,    "hero_image": hero_image.asset->url,    "hero_alt": hero_image.alt,    "content_image": content_image.asset->url,    "content_alt": content_image.alt,    content,    "treatments": *[_type == "treatments" && service._ref == ^._id && isActive == true] | order(coalesce(displayOrder, 9999) asc, title asc){      _id,      title,      "slug": treatmentSlug.current,      "rawSlug": treatmentSlug    },    hero_secondary_title,    hero_large_text,    referral_form_pdf {      asset-> {        url,        originalFilename,      }    },    block_2_title,    block_2_content,    block_2_image {      asset-> {        url,      }    },    block_3_title,    block_3_content,    "block_4_image": block_4_image.asset->url,    "block_5_image": block_5_image.asset->url,    benefits[] {      title,      description,      "image": image.asset->url,      tint_percentage,      tint_percentage_hover    },    "block_7_image": block_7_image.asset->url,    "block_9_image": block_9_image.asset->url,    timeline[] {      title,      description    },    "block_11_image": block_11_image.asset->url,    faq[] {      title,      description    },    call_to_action,    pillars[] {      title,      description    },      seo{    pageTitle,    pageDescription,    socialMeta{      title,      description,      ogImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      },      twitterImage{        crop,        hotspot,        asset-> {          _id,          url,          alt        }      }    }  },    "ourTeam": *[_type == "ourTeam" && pageActive == true][0]{      teamMembers[!(name in *[_type == "practitioner" && isActive == false].name)] {        name,        role,        bio,        image {          asset-> {            url          }        }      }    },    testimonials[] {      name,      description,      image {        asset-> {          url        }      }    }  }
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    '\n*[_type == "practitioner" && isActive == true && defined(slug.current)].slug.current': PRACTITIONER_SLUGS_QUERYResult;
    ...
    '*[_type == "ourProgramsSection"][0]{\n  sectionTitle,\n  "bgImage": {\n    "asset": bgImage.asset->{\n      _id,\n      url\n    },\n    "alt": bgImage.alt\n  },\n  programs[]{\n    name,\n    href,\n    barColor,\n    isLink\n  },\n  hoverLinkText,\n  hoverLinkHref\n}': OUR_PROGRAMS_SECTION_QUERYResult;
